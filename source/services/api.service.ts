...
export class ApiService {
    ...
    private async getSid(): Promise<TSid> {
        let sid: TSid = await StorageService.get(STORAGE_KEYS.TRANSLATOR_SESSION_ID);
        const now = new Date();

        if (!sid || (now.getTime() - new Date(sid.date).getTime()) > 0) {
    ...
    async translateWithExamples(originalText: string): Promise<TranslateWithExampleResult[]> {
    ...
    async translate(originalText: string): Promise<TranslateWithExampleResult[]> {
        ...
        const { data: { result } } = await translate(originalText, id, langFrom.symbol, langTo.symbol);
